...
fs = [
    [f] + re.findall("\d{4}_\d{2}\.png", f)[0].replace(".png", "").split("_")
    for f in fs
]
    ...
    volume += [
        np.concatenate(
            [
                np.concatenate(
                    [
                        cv2.imread(images[0], cv2.IMREAD_GRAYSCALE),
                ...
                np.concatenate(
                    [
                        cv2.imread(images[2], cv2.IMREAD_GRAYSCALE),
                        cv2.imread(images[3], cv2.IMREAD_GRAYSCALE),
                    ],
                    axis=1,
                ),
            ],
